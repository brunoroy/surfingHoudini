...
inline Vector3DF Vector3DF::operator-(const Vector3DF &op) 
{
	Vector3DF n = Vector3DF(x,y,z);	
	n.x -=(VTYPE) op.x; 
	n.y -=(VTYPE) op.y; 
	n.z -=(VTYPE) op.z; 
	return n;
...
inline bool Vector3DF::operator== (const Vector3DF b){return (x==b.x && y == b.y && z == b.z);}
...
inline double Vector3DF::Dot(const Vector3DC &v) const			{double dot; dot = (double) x*v.x + (double) y*v.y + (double) z*v.z; return dot;}
inline double Vector3DF::Dot(const Vector3DI &v) const			{double dot; dot = (double) x*v.x + (double) y*v.y + (double) z*v.z; return dot;}
inline double Vector3DF::Dot(const Vector3DF &v) const			{double dot; dot = (double) x*v.x + (double) y*v.y + (double) z*v.z; return dot;}
...
inline double Vector3DF::Length (void) const { double n; n = (double) x*x + (double) y*y + (double) z*z; if (n != 0.0) return sqrt(n); return 0.0; }
