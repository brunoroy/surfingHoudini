...
inline
Vector3DF& MCGrid::getNormal(unsigned int ix, unsigned int iy, unsigned int iz)
{
	int gridIndex = getGridIndex(ix, iy, iz);
	int dataIndex = _vertices[gridIndex];
...
inline
const Vector3DF& MCGrid::getNormal(unsigned int ix, unsigned int iy, unsigned int iz) const
{
	const static Vector3DF defaultNormal = Vector3DF(0,0,0);

	int dataIndex = _vertices[getGridIndex(ix, iy, iz)];
	return (dataIndex > -1) ? _verticesData[dataIndex].normal : defaultNormal;
}
								  ...
								  std::vector<Vector3DF>& points) const
{
	...
	if (pointID == -1)
	{
		...
		if (t<0 || t>1.0)
		{
			std::cout << "t=" << t << "(" << va->value << ", " << vb->value << ")" << std::endl;
		}
		posA *= 1.0-t;
		posB *= t;
								  ...
								  std::vector<Vector3DF>& normals) const
{
	...
	if (pointID == -1)
	{
		...
		// Compute normal
		Vector3DF normalA = getNormal(getXIndex(va->gridIndex),
									  getYIndex(va->gridIndex),
									  getZIndex(va->gridIndex));
		normalA *= 1.0-t;

		Vector3DF normal = getNormal(getXIndex(vb->gridIndex),
									getYIndex(vb->gridIndex),
									getZIndex(vb->gridIndex));
		normal *= t;
		normal += normalA;
		...
		// add normal
		normals.push_back(normal);	// We assume that size of normals was equal to size
									// of points before the calling this function!
	}
